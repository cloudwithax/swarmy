...
func TestCrushIgnore(t *testing.T) {
	...
	// Create a .swarmyignore file that ignores .log files
	require.NoError(t, os.WriteFile(".swarmyignore", []byte("*.log\n"), 0o644))
...
func TestShouldExcludeFile(t *testing.T) {
	...
	// Create .swarmyignore file
	swarmyignoreContent := "custom_ignored/\n"
	if err := os.WriteFile(filepath.Join(tempDir, ".swarmyignore"), []byte(swarmyignoreContent), 0o644); err != nil {
		t.Fatalf("Failed to create .swarmyignore: %v", err)
	}
	...
	require.True(t, ShouldExcludeFile(tempDir, customIgnored), "Expected custom_ignored to be ignored by .swarmyignore")
...
func TestShouldExcludeFileHierarchical(t *testing.T) {
	...
	// Create .swarmyignore in subdir that ignores normal_nested
	subCrushignore := "normal_nested/\n"
	if err := os.WriteFile(filepath.Join(subDir, ".swarmyignore"), []byte(subCrushignore), 0o644); err != nil {
		t.Fatalf("Failed to create subdir .swarmyignore: %v", err)
	}

	// Test hierarchical ignore behavior - this should work because the .swarmyignore is in the parent directory
	require.True(t, ShouldExcludeFile(tempDir, nestedNormal), "Expected normal_nested to be ignored by subdir .swarmyignore")
